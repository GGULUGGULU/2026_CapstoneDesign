...
void CGameFramework::ProcessInputGameStage()
{
	...
	if (!bProcessedByScene)
	{
		...
		if (m_bIsDrifting && m_nPlayerCurrentSpeed > 35 && s_fDriftSparkTimer >= 0.06f)
		{
			...
			EFFECT_TYPE driftSparkType;

			if (bLeft)
			{
				driftSparkType = EFFECT_TYPE::DRIFT_SPARK_LEFT;
			}
			else if (bRight)
			{
				driftSparkType = EFFECT_TYPE::DRIFT_SPARK_RIGHT;
			}
			else
			{
				return;
	...
	if (2 == m_nStage)
	{
		...
		if (bOnGround)
		{
			...
			if (!m_bBananaSpinning)
			{
				...
				float fYRight = f3Pos.y;
				float fYLeft = f3Pos.y;
...
void CGameFramework::UpdateBananaSpin(float fTimeElapsed)
{
	if (m_fBananaCollisionCooldown > 0.0f)
	{
		m_fBananaCollisionCooldown -= fTimeElapsed;
